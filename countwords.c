...
void __TreeMap_del(struct TreeMap* self) {
    struct TreeMapEntry *current = self->__head;
    struct TreeMapEntry *next; /* Declare next as a pointer */

    while (current) {
        free(current->key);
        next = current->__next; /* Use next to store the next entry */
        free(current);
        current = next; /* Move to the next entry */
    }
    free(self); /* Free the TreeMap structure itself */
}
